...
pub type Session {
  Session(reference: erlang.Reference)
}
...
pub type PreKeyBundle {
  PreKeyBundle(
    ...
    pre_key: #(Int, BitString),
    signed_pre_key: #(Int, BitString, BitString),
    base_key: BitString,
  )
}
...
pub type IdentityKeyPair {
  IdentityKeyPair(public_key: BitString, signature: BitString)
}

/// Represents a pre-key.
pub type PreKey {
  PreKey(key_id: Int, public_key: BitString)
}

/// Represents a signed pre-key.
pub type SignedPreKey {
  SignedPreKey(key_id: Int, public_key: BitString, signature: BitString)
}
...
pub fn generate_identity_key_pair() -> Result(IdentityKeyPair, String) {
  ...
  {
    Ok(#(Ok, #(public_key, signature))) ->
      Ok(IdentityKeyPair(public_key: public_key, signature: signature))
...
pub fn generate_pre_key(key_id: Int) -> Result(PreKey, String) {
  case
    erlang.call_function("Elixir.SignalProtocol", "generate_pre_key", [key_id])
  {
    Ok(#(Ok, #(key_id, public_key))) ->
      Ok(PreKey(key_id: key_id, public_key: public_key))
    Ok(#(Error, reason)) -> Error(erlang.bit_string_to_string(reason))
    Error(e) -> Error(erlang.bit_string_to_string(e))
  }
}
...
pub fn generate_signed_pre_key(
  ...
  case
    erlang.call_function("Elixir.SignalProtocol", "generate_signed_pre_key", [
      identity_key,
      key_id,
    ])
  {
    Ok(#(Ok, #(key_id, public_key, signature))) ->
      Ok(SignedPreKey(
        key_id: key_id,
...
pub fn create_session(
  ...
  case
    erlang.call_function("Elixir.SignalProtocol", "create_session", [
      local_identity_key,
      remote_identity_key,
    ])
  {
...
pub fn process_pre_key_bundle(
  ...
  case
    erlang.call_function("Elixir.SignalProtocol", "process_pre_key_bundle", [
      session.reference,
      bundle_binary,
    ])
  {
...
pub fn encrypt_message(
  ...
  case
    erlang.call_function("Elixir.SignalProtocol", "encrypt_message", [
      session.reference,
      message,
    ])
  {
...
pub fn decrypt_message(
  ...
  case
    erlang.call_function("Elixir.SignalProtocol", "decrypt_message", [
      session.reference,
      ciphertext,
    ])
  {
