...
-export([
    run_benchmarks/0,
    benchmark_encryption/1,
    benchmark_decryption/1,
    benchmark_key_generation/1,
    benchmark_cache_performance/1,
    benchmark_memory_usage/1,
    benchmark_concurrent_operations/1,
    generate_report/1
]).
...
run_benchmarks() ->
    ...
    Results = [
        {"Key Generation", benchmark_key_generation(1000)},
        {"Encryption", benchmark_encryption(10000)},
        {"Decryption", benchmark_decryption(10000)},
        {"Cache Performance", benchmark_cache_performance(1000)},
        {"Memory Usage", benchmark_memory_usage(1000)},
        {"Concurrent Operations", benchmark_concurrent_operations(100)}
    ],
...
benchmark_key_generation(Iterations) ->
    ...
    Keys = [libsignal_protocol_nif:generate_identity_key_pair() || _ <- lists:seq(1, Iterations)],
    ...
    #{duration => Duration, avg_time => AvgTime, throughput => Throughput}.
...
benchmark_encryption(Iterations) ->
    ...
    EncryptedMessages = [
        begin
            Message = lists:nth(rand:uniform(length(TestMessages)), TestMessages),
...
benchmark_decryption(Iterations) ->
    ...
    DecryptedMessages = [
        begin
            Encrypted = lists:nth(rand:uniform(length(EncryptedMessages)), EncryptedMessages),
...
benchmark_cache_performance(Iterations) ->
    ...
    % Generate test data
    TestData = [{crypto:strong_rand_bytes(32), crypto:strong_rand_bytes(1024)} 
                || _ <- lists:seq(1, 100)],
    ...
    [begin
        {Key, Value} = lists:nth(rand:uniform(length(TestData)), TestData),
...
benchmark_memory_usage(Iterations) ->
    ...
    #{duration => Duration, avg_time => AvgTime, throughput => Throughput, memory_diff => MemoryDiff}.
...
benchmark_concurrent_operations(Iterations) ->
    ...
    Results = pmap(fun(_) ->
        ...
        {ok, Decrypted} = libsignal_protocol_nif:decrypt_message(Session, Encrypted),
        {Encrypted, Decrypted}
    end, lists:seq(1, Iterations), 10), % 10 concurrent processes
...
generate_report(Results) ->
    ...
    AvgThroughput = lists:sum([maps:get(throughput, Result) || {_, Result} <- Results]) / length(Results),
    ...
    io:format(File, "Generated: ~s~n~n", [calendar:system_time_to_rfc3339(erlang:system_time(second))]),
    ...
    [begin
        ...
        io:format(File, "  Throughput: ~.0f operations/second~n~n", [maps:get(throughput, Result)])
    end || {Name, Result} <- Results],
